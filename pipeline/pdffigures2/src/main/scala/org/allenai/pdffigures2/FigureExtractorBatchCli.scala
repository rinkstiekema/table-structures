...
object FigureExtractorBatchCli extends Logging {
  ...
  case class CliConfigBatch(
                             ...
                             figureDataPrefix: Option[String] = None, dpi: Int = 72, ignoreErrors: Boolean = false,
                             ...
                             figureImagePrefix: Option[String] = None, figureFormat: String = "png", figType: String = "All"
                           )
  ...
  val Parser = new scopt.OptionParser[CliConfigBatch]("figure-extractor-batch") {
    ...
    opt[String]('a', "figType") action { (a, c) =>
      c.copy(figType = a)
    } validate { a =>
      if (a == "All" || a == "Table" || a == "Figure") success else failure("Figure type must either be all, table or figure")
    }
  ...
  def saveRasterizedFigures(prefix: String, docName: String, format: String, dpi: Int,
                            figures_ : Seq[RasterizedFigure], doc: PDDocument, figType: String): Seq[SavedFigure] = {
    var figures = figures_
    if(figType != "All"){
      figures = figures.filter(_.figure.figType.id == figType)
    }
      ...
      val numFigures = if (config.fullTextPrefix.isDefined) {
        ...
        val numFigures = if (config.figureImagePrefix.isDefined && !useCairo) {
          ...
          val savedFigures = saveRasterizedFigures(config.figureImagePrefix.get, truncatedName,
            config.figureFormat, config.dpi, document.figures, doc, config.figType)
        ...
        val (figures, failedCaptions) = if (config.figureImagePrefix.isDefined && !useCairo) {
          ...
          val savedFigures = saveRasterizedFigures(config.figureImagePrefix.get, truncatedName,
            config.figureFormat, config.dpi, figuresWithErrors.figures, doc, config.figType)
